...
function HighlightedText({
  ...
  indices.forEach(([start, end], index) => {
    ...
    parts.push(
      <mark
        key={`${start}-${end}-${index}`}
        className="rounded bg-cyan-500/25 px-0.5 text-cyan-300 shadow-[0_0_12px_rgba(34,211,238,0.35)]"
      >
...
export default function ProjectCard({
  ...
  const descriptionMatch = project.matches?.find(
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex justify-between items-center mb-3">
          <p className="text-xs text-primary font-medium">
            by {project.authorName || 'Anonymous'}
          </p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {project.skills.slice(0, 3).map((skill) => {
              ...
              return (
                <span
                  key={skill}
                  className="px-2 py-0.5 bg-secondary/50 text-secondary-foreground text-[10px] rounded-full border border-border/50"
                >
