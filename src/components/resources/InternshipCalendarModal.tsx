...
export function InternshipCalendarModal({
  isOpen,
  onClose,
}: InternshipCalendarModalProps) {
  const { user } = useAuth();
      ...
      if (docSnap.exists()) {
        ...
        if (user) {
          ...
          if (userSnap.exists()) {
            ...
            setHasStarred(
              starredResources.includes('Internship_Calendar_2026')
            );
          }
        }
      }
    ...
    try {
      ...
      if (hasStarred) {
        await Promise.all([
          ...
          updateDoc(userRef, {
            starredResources: arrayRemove('Internship_Calendar_2026'),
          }),
        ]);
        ...
        await Promise.all([
          updateDoc(resourceRef, { starCount: increment(1) }),
          updateDoc(userRef, {
            starredResources: arrayUnion('Internship_Calendar_2026'),
          }),
        ]);
        setStarCount((prev) => prev + 1);
        setHasStarred(true);
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 space-y-3">
                            ...
                            {/* Links */}
                            <div
                              className={`grid gap-3 ${internship.links.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}
                            >
