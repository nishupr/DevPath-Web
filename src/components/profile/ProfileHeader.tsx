    ...
    <header className="relative border-b border-white/10 bg-gradient-to-b from-[#1A1D25] to-[#0D0F14]">
      <div className="mx-auto max-w-5xl px-4 py-12 flex flex-col sm:flex-row items-center sm:items-end gap-6">
        ...
        <div className="relative flex-shrink-0">
          ...
          <span className="absolute -bottom-1 -right-1 bg-[#4F9EFF] rounded-full p-1">
            <svg className="w-3 h-3 text-white" viewBox="0 0 12 12" fill="currentColor">
              <path d="M10.5 3L5 8.5 2 5.5" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </span>
        </div>
        ...
        <div className="text-center sm:text-left flex-1">
          ...
          {tagline && (
            <p className="mt-1 text-gray-400 max-w-md">{tagline}</p>
          )}
        </div>
        ...
        <nav className="flex gap-3">
          ...
          {socials.twitter && (
            <SocialLink href={`https://twitter.com/${socials.twitter.replace("@","")}`} label="Twitter">
              <Twitter size={18} />
            </SocialLink>
          )}
          {socials.website && (
