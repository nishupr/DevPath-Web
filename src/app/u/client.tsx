              ...
              <div className="mb-2 flex flex-wrap gap-2">
                ...
                {user.communityRoles &&
                  user.communityRoles.map((role, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 text-xs font-bold border border-blue-200 dark:border-blue-800"
                    >
                      <Shield size={12} /> {role}
                    </span>
                  ))}
            ...
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                <div className="flex flex-col items-center p-4 bg-muted/30 rounded-xl border border-border/50 hover:border-primary/50 transition-colors">
                  ...
                  <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider text-center">
                    Repositories
                  </span>
                </div>
                <div className="flex flex-col items-center p-4 bg-muted/30 rounded-xl border border-border/50 hover:border-primary/50 transition-colors">
                    ...
                    {project.skills && project.skills.length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-4 h-6 overflow-hidden">
                        {project.skills.slice(0, 3).map((skill) => (
                          <span
                            key={skill}
                            className="text-[10px] px-1.5 py-0.5 bg-secondary text-secondary-foreground rounded-md"
                          >
                            {skill}
                          </span>
                        ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {user.achievements && user.achievements.length > 0 ? (
                user.achievements.map((badgeId, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary hover:scale-105 transition-all duration-200"
                  >
