...
export default function CommunityPage() {
  ...
  const [activeTab, setActiveTab] = useState<'discussions' | 'showcase'>(
    'discussions'
  );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (error.message.includes('requires an index')) {
        alert(
          'Missing Index! Please check the console for the link to create it.'
        );
      }
    } finally {
      setLoading(false);
                ...
                {discussions.length === 0 ? (
                  ...
                  discussions.map((discussion) => (
                    <div
                      key={discussion.id}
                      onClick={() =>
                        router.push(`/community/view?id=${discussion.id}`)
                      }
