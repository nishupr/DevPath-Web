...
const REACTIONS = [
  ...
  { emoji: '🥺', label: 'Cute', icon: <div className="text-xl">🥺</div> },
  {
    emoji: '🤝',
    label: 'Handshake',
    icon: <Handshake className="text-amber-600" size={20} />,
  },
  {
    emoji: '💡',
    label: 'Insightful',
    icon: <Lightbulb className="text-yellow-500 fill-yellow-500" size={20} />,
  },
  {
    emoji: '🎉',
    label: 'Celebrate',
...
export default function DiscussionViewClient() {
  ...
  useEffect(() => {
      ...
      try {
        if (!db || discussionId === 'mock-discussion-1') {
          setDiscussion({
            ...
            content:
              'This is a mock discussion content to test Markdown formatting: \n\n- List item\n- **Bold text**\n- `inline code`',
            tags: ['General', 'Help'],
            reactions: {},
          });
          setLoading(false);
    ...
    const unsubDiscussion = onSnapshot(
  ...
  const handleShare = async () => {
    ...
    if (copiedSuccessfully) {
      ...
      showError(
        'Copying the discussion link is not supported in this browser.'
      );
    }
  };
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6 pb-6 border-b border-border">
            ...
            <span>
              {discussion.createdAt?.seconds
                ? new Date(
                    discussion.createdAt.seconds * 1000
                  ).toLocaleDateString()
                : 'Just now'}
            </span>
          </div>
        ...
        <div className="space-y-6">
          ...
          {user ? (
            <form
              ...
              className="bg-card border border-border rounded-xl p-4 mt-8"
            >
